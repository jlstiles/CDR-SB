...
random_effects = list(Y_0 = list(dist = runif, params = list(min = 1, max = 4)),
                      ...
                      rate_A = list(dist = beta_s, params = list(shape1 = 3,
                                                                 shape2 = 5, fac = -.5, addon = .085 ))
)


effects = list(f1 = function(W) {
  L = length(W[[1]])
  re = rnorm(mean = .06, sd = .1, n = L)
...
f4 = function(W) {
  ...
  e = unlist(lapply(1:L, FUN = function(x) (W$W4[x]-15)/4*time*re[x]*.7))
  return(e)
},
f5 = function(W) {
  L = length(W[[1]])
  re = rbeta(shape1 = 2, shape2 = 1, n = L) - 2/3 + .01
  e = unlist(lapply(1:L, FUN = function(x) (W$W5[x]-70)/7.5*time*re[x]*.1*.7))
  return(e)
},
f6 = function(W) {
  L = length(W[[1]])
...
save(test, file = "test2_interim.RData")
